...
inline size_t __deque_buf_size(size_t n, size_t sz)                                                                           //返回给定buffer_size能够存储的元素个数
{
...
template <class T, class Alloc = alloc, size_t BufSiz = 0>                                                                                 //deque的类定义，deque的结构和一般的不一样，它分为两部分一部分是map，另一部分是buffer，这个简单的结构可以想想为一个文件系统，map是文件系统的索引，而buffer是实际的数据内容
class deque {
public:                         // Basic types
  typedef T value_type;
...
public:                         // Constructor, destructor.
  deque()
    : start(), finish(), map(0), map_size(0)                                                                        //初始化一个空的deque
  {
...
public:                         // push_* and pop_*
  ...
  void push_back(const value_type& t) {
    if (finish.cur != finish.last - 1) {                                                        //如果数据能够直接插入当前的缓冲区则直接插入，否则需要进行索引节点的调整
  ...
  void push_front(const value_type& t) {                                                       //如果数据能够直接删除则删除，否则需要进行索引节点的调整，下面类似
...
protected:                      // Allocation of map and nodes
  ...
  void reserve_map_at_back (size_type nodes_to_add = 1) {                                           //在当前数据结尾扩充空间
    if (nodes_to_add + 1 > map_size - (finish.node - map))
      reallocate_map(nodes_to_add, false);
  }

  void reserve_map_at_front (size_type nodes_to_add = 1) {                                          //在当前数据开头扩充空间
  ...
  pointer allocate_node() { return data_allocator::allocate(buffer_size()); }                                 //分配一个缓冲区大小的内存
  void deallocate_node(pointer n) {                                                                           //销毁一个缓冲区大小的内存
...
template <class T, class Alloc, size_t BufSize>
void deque<T, Alloc, BufSize>::insert(iterator pos,                             //基本情况同上
...
deque<T, Alloc, BufSize>::erase(iterator first, iterator last) {
  if (first == start && last == finish) {
    ...
    if (elems_before < (size() - n) / 2) {                                      //这里进行判断，是为了选择相对来说移动数据较少的一侧，进行数据操作，即如果删除位置前半段数据少于后半段则操作前半段移动，否则操作后半段移动
...
template <class T, class Alloc, size_t BufSize>
void deque<T, Alloc, BufSize>::clear() {                                                                                          //清空当前维护的所有内存
...
template <class T, class Alloc, size_t BufSize>
void deque<T, Alloc, BufSize>::destroy_map_and_nodes() {                                          //销毁当前维护的索引结点和数据内容
...
template <class T, class Alloc, size_t BufSize>
void deque<T, Alloc, BufSize>::push_back_aux(const value_type& t) {                                             //在末尾插入元素
  value_type t_copy = t;
  reserve_map_at_back();
...
template <class T, class Alloc, size_t BufSize>
void deque<T, Alloc, BufSize>::push_front_aux(const value_type& t) {                                          //在开头插入元素
  value_type t_copy = t;
...
template <class T, class Alloc, size_t BufSize>
void deque<T, Alloc, BufSize>:: pop_back_aux() {                                                            //删除末尾的元素
...
//  at least two nodes.
template <class T, class Alloc, size_t BufSize>
void deque<T, Alloc, BufSize>::pop_front_aux() {                                                          //删除开头的元素
  destroy(start.cur);
...
void deque<T, Alloc, BufSize>::insert(iterator pos,
                                      ...
                                      ForwardIterator last,
                                      forward_iterator_tag) {                                                   //插入元素和之前的erase等操作类似，判断在开头末尾还是中间进行数据操作
  size_type n = 0;
  distance(first, last, n);
  if (pos.cur == start.cur) {
    iterator new_start = reserve_elements_at_front(n);
...
void deque<T, Alloc, BufSize>::insert_aux(iterator pos,
  ...
  value_type x_copy = x;
  if (elems_before < length / 2) {                                                            //这里进行判断，是为了选择相对来说移动数据较少的一侧，进行数据操作，即如果删除位置前半段数据少于后半段则操作前半段移动，否则操作后半段移动
    iterator new_start = reserve_elements_at_front(n);
