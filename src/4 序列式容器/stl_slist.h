...
private:
  ...
  static list_node* create_node(const value_type& x) {          //创建一个值为x的结点
  ...
  static void destroy_node(list_node* node) {               //销毁结点
    destroy(&node->data);
...
private:
  list_node* _insert_after(list_node_base* pos, const value_type& x) {          //结点之后插入
    return (list_node*) (__slist_make_link(pos, create_node(x)));
  }

  void _insert_after_fill(list_node_base* pos,
                          size_type n, const value_type& x) {                 //结点之后插入n个值
  ...
  list_node_base* erase_after(list_node_base* pos) {          //删除之后的结点
...
public:
  ...
  iterator insert_after(iterator pos, const value_type& x) {              //当前位置之后插入
