...
/*
 * @brief 因为链表的内存操作比较简单，主要是对指针的调整，很少涉及内存的申请除了创建或者销毁结点，故大部分并不做注释，很容易明白
 */
template <class T>
...
public:
  ...
  iterator insert(iterator position, const T& x) {                                                                                //在position处插入一个节点并初始化为x
  ...
  iterator erase(iterator position) {                                                                                           //删除position处的结点
...
protected:
  void transfer(iterator position, iterator first, iterator last) {                                                       //将[first, last)移动到postion之前
    if (position != last) {
...
template <class T, class Alloc>
void list<T, Alloc>::merge(list<T, Alloc>& x) {                                                         //合并链表
  iterator first1 = begin();
  iterator last1 = end();
...
template <class T, class Alloc>
void list<T, Alloc>::sort() {                                                                             //对链表进行排序，使用快速排序
  if (node->next == node || link_type(node->next)->next == node) return;
  list<T, Alloc> carry;
