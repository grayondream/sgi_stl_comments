...
/*
 * @brief  找到符合和第一个元素符合binary_pred的元素
 */
...
/*
 * @brief  找到[first2, last2)在[first1, last1)中的位置
 */
...
/*
 * @brief  对两个范围内的元素执行binary_op并存储在result
 */
...
/*
 * @brief  当符合条件pred时将元素替换为new_value
 */
...
/*
 * @brief 数据量大时采用QuickSort快排算法，分段归并排序。一旦分段后的数据量小于某个门槛（16），为避免QuickSort快排的递归调用带来过大的额外负荷，就改用Insertion Sort插入排序。如果递归层次过深，还会改用HeapSort堆排序
 */
template <class RandomAccessIterator>
...
/*
 * @brief 排列组合的下一个按顺序的下一个元素 1234 -> 1243 -> 1324
 */
template <class BidirectionalIterator>
