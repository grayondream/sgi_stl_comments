...
struct __rb_tree_iterator : public __rb_tree_base_iterator                  //rbtree迭代器的具体实现
{
...
public:
  ...
  typedef ptrdiff_t difference_type;
protected:
  link_type get_node() { return rb_tree_node_allocator::allocate(); }                            //内存空间分配
  void put_node(link_type p) { rb_tree_node_allocator::deallocate(p); }                          //销毁空间

  link_type create_node(const value_type& x) {                                                   //创建结点，并将结点初始化为x
  ...
  link_type clone_node(link_type x) {                                                            //clone结点，只拷贝数据，不拷贝指针连接关系
  ...
  void destroy_node(link_type p) {                                                              //销毁结点
...
private:
  ...
  void init() {
    header = get_node();
    color(header) = __rb_tree_red; // used to distinguish header from               //用来区分方便访问，作为iterator::operator++访问的一种特殊情况，根节点必为黑色
